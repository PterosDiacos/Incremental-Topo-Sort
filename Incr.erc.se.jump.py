# DEFINE
FILE_PARAMETER = "c160-e12000.txt"
import time, sys
...
fout = open("out.erc.se.jump-" + FILE_PARAMETER, "w")
...
for line in matrix:
    ...
    if firstTimeFlag:
        ...
        if len(lset) > 0:
            xindex = topsort.index(list(lset)[0])
            rbound = xindex
            for l in lset:
                if topsort.index(l) < xindex: xindex = topsort.index(l)
                if topsort.index(l) > rbound: rbound = topsort.index(l)
            if rbound < len(topsort) - 1: rbound += 1
